  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-container" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-content">
          <p>Você  realmente deseja remover o evento/ponto turístico solicitado da sua lista de favoritos?</p>
        </div>
        <div className="modal-actions">
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const favoritesWithDetails = await Promise.all(data.map(async (fav) => {
                        if (fav.type && fav.id) {
                            const detailResponse = await fetch(`http://localhost:8080/culturais/${fav.type}/${fav.id}`);
            ...
            <section className="screen" id="tela-profile">
                ...
                <div className="favorites-box">
                    ...
                    <div className="favorites-list">
                        {favorites.length > 0 ? (
                            favorites.map(fav => (
                               (fav.Title && fav.Image && fav.Location) && (
                                <div key={fav.id} className="favorite-card">
                                    <Link to={`/card/${fav.id}`} state={{ userID, userType, isEvent: fav.type === 'event' }} className="card-link">
                                        <img src={`/thumb-size/${fav.Image}`} alt={fav.Title} className="favorite-img" />
                                        <div className="favorite-details">
                                            <h3>{fav.Title}</h3>
                                            <p>{fav.Type === 'event' ? 'Evento' : 'Ponto Turístico'}</p>
                                            <span>{fav.Location}</span>
                                            <span className="price">{fav.Price === "R$0,00" || fav.Price === "Gratuito" ? "Gratuito" : `${fav.Price}`}</span>
                                        </div>
                                    </Link>
