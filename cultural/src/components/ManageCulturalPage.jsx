  ...
  useEffect(() => {
        const fetchCulturais = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    const culturaisWithDetails = await Promise.all(data.map(async (cult) => {
                        if (cult.type && cult.id) {
                            const detailResponse = await fetch(`http://localhost:8080/culturais/${cult.type}/${cult.id}`);
    ...
    <section className="screen" id="tela-profile">
      ...
      <div className="profile-box">
        ...
        <div className="favorites-list">
          {culturais.length > 0 ? (
            culturais.map(cult => (
              (cult.Title && cult.Image && cult.Location) && (
                <div key={cult.id} className="favorite-card">
                  <Link to={`/card/${cult.id}`} state={{ userID, userType, isEvent: cult.type === 'event' }} className="card-link">
                    <img src={`/thumb-size/${cult.Image}`} alt={cult.Title} className="favorite-img" />
                    <div className="favorite-details">
                      <h3>{cult.Title}</h3>
                      <p>{cult.Type === 'event' ? 'Evento' : 'Ponto Turístico'}</p>
                      <span>{cult.Location}</span>
                      <span className="price">{cult.Price === "R$0,00" || cult.Price === "Gratuito" ? "Gratuito" : `${cult.Price}`}</span>
                    </div>
                  </Link>
                  <button className="remove-btn">Remover</button>
                </div>)))) : 
            (
              <p><em>Você ainda não adicionou nenhum cultural.</em></p>
          )}
        </div>
        <div className="profile-actions">
          <div className="profile-actions-row">
            <button onClick={handleGoBackClick} className="profile-btn">Voltar</button>
          </div>
        </div>
      </div>
